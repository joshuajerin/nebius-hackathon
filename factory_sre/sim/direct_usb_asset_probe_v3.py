"""Final structure probe for the reduced asset set and plain wrist camera."""
...
def _inspect(stage, root_path: str) -> dict[str, object]:
    ...
    prims = list(Usd.PrimRange(root)) if root.IsValid() else []
    return {
        "valid": root.IsValid(),
...
def main() -> None:
    ...
    for index, (name, path) in enumerate(STOCK_ASSETS.items()):
        prim_path = f"/World/AssetProbeV3/A{index}_{name}"
    ...
    try:
        antioch.load_asset("so101_antioch", prim_path="/World/AssetProbeV3/SO101", version="1.3.2")
        wrist_camera = UsdGeom.Camera.Define(stage, "/World/AssetProbeV3/SO101/PlainWristCamera")
        wrist_camera.CreateFocalLengthAttr(2.8)
        wrist_camera.CreateHorizontalApertureAttr(4.8)
        wrist_camera.CreateVerticalApertureAttr(3.6)
        wrist_camera.CreateClippingRangeAttr((0.03, 10.0))
        stage.Load()
        for _ in range(2):
            app_utils.update_app()
        report["so101"] = {
            "name": "so101_antioch",
            "version": "1.3.2",
            "plain_wrist_camera": str(wrist_camera.GetPath()),
            **_inspect(stage, "/World/AssetProbeV3/SO101"),
        }
    except Exception as exc:
